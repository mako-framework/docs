...
> When working with [PostgreSQL](http://www.postgresql.org), the ```insertAndGetId``` assumes that the sequence id follows the default naming convention (```<table_name>_<primary_key>_seq```) You can override the default primary key name (```id``) by using the optional second parameter.
