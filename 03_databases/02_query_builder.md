...
Fetching all rows is done using the ```Query::all``` method.
	...
	$persons = $connection->builder()->from('persons')->all();

	// You can also specify which columns you want to include in the result set

	$persons = $connection->builder()->select(['name', 'email'])->from('persons')->all();

	// To make a distinct selection use the distinct method

	$persons = $connection->builder()->select(['name', 'email'])->distinct()->from('persons')->all();
...
Selecting from the results of a subquery is also possible.
	...
	$persons = $connection->builder()->from
	(
		new Subquery
		(
...
You can also make advanced column selections with raw SQL and subqueries
	...
	$persons = $connection->builder()->select
	(
		[
			'name',
			'email',
...
If you only want to retrieve a single row you can use the ```Query::first``` method.
	...
	$person = $connection->builder()->from('persons')->where('id', '=', 1)->first();

Fetching the value of a single column is done using the ```Query::column``` method.

	$email = $connection->builder()->select(['email'])->from('persons')->where('id', '=', 1)->column();
...
You can also set the offset starting point and offset end point using the optional third and fourth parameters respectively. This is useful if you have parallel workers processing data.
	...
	$connection->builder()->from('persons')->ascending('id')->batch(function($batch)
	{
...
The query builder also includes a few handy shortcuts to the most common aggregate functions:
	...
	$count = $connection->builder()->from('persons')->count();

	$count = $connection->builder()->from('persons')->where('age', '>', 25)->count();

	// Average value

	$height = $connection->builder()->from('persons')->avg('height');

	$height = $connection->builder()->from('persons')->where('age', '>', 25)->avg('height');

	// Largest value

	$height = $connection->builder()->from('persons')->max('height');

	$height = $connection->builder()->from('persons')->where('age', '>', 25)->max('height');

	// Smallest value

	$height = $connection->builder()->from('persons')->min('height');

	$height = $connection->builder()->from('persons')->where('age', '>', 25)->min('height');

	// Sum

	$height = $connection->builder()->from('persons')->sum('height');

	$height = $connection->builder()->from('persons')->where('age', '>', 25)->sum('height');
...
where(), whereRaw(), orWhere(), orWhereRaw()
	...
	$persons = $connection->builder()->from('persons')
	->where('age', '>', 25)
	->all();
...
join(), joinRaw(), leftJoin(), leftJoinRaw()
	...
	$persons = $connection->builder()->from('persons as u')
...
groupBy()
	...
	$customers = $connection->builder()->from('orders')
	->groupBy('customer')
...
You can also use a [pagination instance](:base_url:/docs/:version:/learn-more:pagination) to limit your results.
	...
	$persons = $connection->builder()->from('persons')->paginate($pagination)->all();