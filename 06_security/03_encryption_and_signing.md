...
> Note: We're using the `Key::decode()` method since we're assuming that your application secret and encryption key were generated by the `app:generate-secret` and `app:generate-key` reactor commands.
